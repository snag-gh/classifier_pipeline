...
if(length(args) == 0) {
        stop("USAGE: ./classifier.R /path/to/samplesheet /path/to/output.pdf sampleID sentrix_id /path/to/pipeline version currentdate\n", call.=FALSE)
} 
...
MNPreport(RGsetEpic, sample=args[4], sampleID=args[3], case=sheet$Surgical_Case, material=sheet$Material_Type, sex=sheet$Gender, arrayt=sheet$Sample_Group, diagnosis=sheet$Diagnosis, output_file=output_file, output_dir=output_dir, pipeline=args[5], version=args[6], currentdate=args[7])
